...
use substrate_archive_common::{
    types::{Block, Extrinsic, BatchExtrinsic, BatchBlock},
    Result,
};
            ...
            let _ = UncheckedExtrinsic::decode(&mut extrinsic.encode().as_ref()).map(|e| {
                ...
                match e.signature {
                    Some((address, signature, extra)) => {
                        extrinsics_list.push(Extrinsic::new_signed(hash, block_num, index as u32, address.encode(), signature.encode(), extra.encode(), function.encode()));
                    }
                    None => {
